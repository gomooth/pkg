...
func New(opts ...func(*types.Option)) xlog.XLogger {
	return NewWith(types.GetDefaultDir(), types.GetDefaultCategory(), opts...)
}

func NewWith(path, category string, opts ...func(*types.Option)) xlog.XLogger {
	l := &logger{}

	myPath := types.GetDefaultDir()
	if len(path) > 0 {
		myPath = strings.TrimRight(path, string(filepath.Separator))
	}
	myCategory := types.GetDefaultCategory()
	if len(category) > 0 {
		myCategory = strings.Trim(category, string(filepath.Separator))
		// 如果 path 中最后已经含了 category，则不重复创建
		myPath = strings.TrimSuffix(myPath, myCategory)
	}

	// 创建目录
	logPath := filepath.Join(myPath, myCategory)

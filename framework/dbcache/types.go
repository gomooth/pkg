...
type IDBCache[E, F any] interface {
	// Paginate 分页列表
	Paginate(ctx context.Context, start, limit int, opt dbfilter.IFilter[F], query func() ([]*E, uint, error)) ([]*E, uint, error)
	// List 列表所有
	List(ctx context.Context, opt dbfilter.IFilter[F], query func() ([]*E, error)) ([]*E, error)
	// First 按 id 查询数据

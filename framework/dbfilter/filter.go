...
func (df *dbFilter[T]) Sorters() []pager.Sorter {
	if df.sorters == nil {
		return []pager.Sorter{}
	}
...
func (df *dbFilter[T]) Preloads() []string {
	if df.preloads == nil {
		return []string{}
	}
...
func (df *dbFilter[T]) Build(db *gorm.DB, opts ...func(*buildOption)) *gorm.DB {
	if df.filter == nil {
		df.filter = new(T)
	}
...
func (df *dbFilter[T]) String() string {

...
type retryStrategy interface {
	OnMessage(ctx context.Context, queue string, data string, pos int64) error
}
...
func handleExhausted(
	...
	if deadLetter != nil {
		if dlErr := deadLetter.OnDeadLetter(ctx, queue, data, pos, lastErr); dlErr != nil {
			if logger != nil {
	...
	if failedHandler != nil {
		failedHandler(ctx, queue, data, pos, lastErr)
	} else if logger != nil {
		logger.Error("event handle failed after retries", "queue", queue, "pos", pos, "error", lastErr)
	}
	return exhaustedContinue
}
...
func (s *syncRetryStrategy) OnMessage(ctx context.Context, queue string, data string, pos int64) error {
	var lastErr error
	...
	// 重试耗尽
	handleExhausted(ctx, queue, data, pos, lastErr,

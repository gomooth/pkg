...
func (t *WatermarkTracker) MarkPending(topic string, partition int32, offset int64) bool {
	...
	if len(s.pending[tp]) >= maxPendingPerPartition {
		t.logger.Warn("watermark: pending set overflow, degrading",
			"topic", topic,
			"partition", int(partition),
			"pending_count", len(s.pending[tp]),
			"max_pending", maxPendingPerPartition,
		)
		return false
	}
...
// noopSlogLogger returns a slog.Logger that discards all output
